...
import { Activity, BriefcaseBusiness, ChevronRight, Copy, Key, Landmark, Plus, Send, ShieldCheck, Users, Wallet } from 'lucide-react';
...
const TEMPLATES: Record<OrganizationPersona, Array<{ label: string; description: string; claims: ClaimType[]; documents: DocumentType[] }>> = {
  hr: [
    ...
    { label: 'Current employment', description: 'Confirm an active employer or role.', claims: ['currently_employed'], documents: ['payslip', 'resume'] },
    { label: 'Employment package', description: 'Check degree, employer, and current role together.', claims: ['degree_verified', 'currently_employed'], documents: ['degree_certificate', 'payslip', 'resume'] },
  ],
  fintech: [
  ...
  marketplace: [
    ...
    { label: 'Active professional', description: 'Confirm an active employer or role.', claims: ['currently_employed'], documents: ['resume', 'payslip'] },
  ],
};
...
export const DeveloperPortal: React.FC = () => {
  ...
  const createRequest = () => {
    ...
    const request = VeriFlowStore.createVerificationRequest({ organization: selectedOrg, persona, subjectReference: subjectReference.trim(), subjectEmail: subjectEmail.trim() || undefined, claims: template.claims, allowedDocumentTypes: template.documents, callbackUrl: callbackUrl.trim() || undefined });
  ...
  return <div className="max-w-7xl mx-auto space-y-7 animate-fade-in">
    <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-5 pb-5 border-b border-slate-800">
      ...
      <div className="flex items-center gap-2"><select value={selectedOrg?.id} onChange={e => { const org = organizations.find(item => item.id === e.target.value); if (org) { setSelectedOrg(org); setCallbackUrl(org.webhookUrl || ''); } }} className="bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs text-slate-200"><option disabled>Select organization</option>{organizations.map(org => <option key={org.id} value={org.id}>{org.name}</option>)}</select><button className="px-3 py-2 rounded-xl bg-teal-500 text-slate-950 text-xs font-black"><Plus className="w-3.5 h-3.5 inline mr-1" />New key</button></div>
    </div>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
      <section className="xl:col-span-7 bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6">
        ...
        <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs"><div className="font-bold text-slate-300">Requested claims</div><div className="flex flex-wrap gap-2 mt-2">{template.claims.map(claim => <span key={claim} className="px-2 py-1 rounded-lg bg-teal-500/10 border border-teal-500/20 text-teal-300 font-mono">{claim}</span>)}</div><div className="font-bold text-slate-300 mt-3">Accepted documents</div><div className="text-slate-500 mt-1">{template.documents.join(', ')}</div></div>
